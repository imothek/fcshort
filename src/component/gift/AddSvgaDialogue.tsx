...
const AddSvgaDialogue = () => {
  ...
  useEffect(() => {
    const loadSvgaPlayerWeb = async () => {
      ...
      if (isSvga) {
        ...
        const element = document.getElementById("svga");
        console.log("element", element);
        if (element && imagePath) {
          const player = new Player(`div[id="svga"]`);
  ...
  const captureAndSendImage = (player: any, mongoId: any) => {
    return new Promise((resolve) => {
      ...
      const canvasElement = document.querySelector(`div[id="svga"] canvas`) as HTMLElement;
      ...
      html2canvas(canvasElement, {
        ...
        onclone: (cloneDoc) => {
          const clonedCanvas = cloneDoc.querySelector(`div[id="svga"] canvas`) as HTMLElement;
          if (clonedCanvas) {
