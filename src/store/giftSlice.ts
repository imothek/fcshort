      ...
      (state, action: PayloadAction<any, string, { arg: AllGiftPayload }>) => {
        if (action.payload.status === true) {
          state.allGift = state.allGift.map((category) => {
            if (category?._id === action.meta.arg?.giftCategoryId) {
              category.gift = category?.gift.filter((gift) => {
                return gift?._id !== action.meta.arg?.giftId;
              });
            }
            return category;
          });
        }
        const deletedUserIds = action.meta.arg?.giftId;
        state.isLoading = false;
      ...
      (state, action: PayloadAction<any, string, { arg: AllGiftPayload }>) => {
        ...
        if (action.payload.data.status === true) {
          ...
          const updatedGiftArray = state.allGift.map((giftCategory) => {
            if (giftCategory?._id === giftCategoryId) {
              const updatedGifts = giftCategory.gift.map((gift) => {
                if (gift?._id === giftId) {
                  return { ...gift, ...action.payload.data?.data };
                }
                return gift;
              });
              return {
