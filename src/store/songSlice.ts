...
export const deleteSong = createAsyncThunk(
    ...
    async (payload: AllSongPayload | undefined) => {
        return apiInstanceFetch.delete(`admin/song/deletesong?songId=${payload?.songId}`);
    }
);
...
export const updateSong = createAsyncThunk(
    ...
    async (payload: AllSongPayload | undefined) => {
        return axios.patch(`admin/song/updateSong?songId=${payload?.songId}`, payload.data);
    }
);
...
export const deleteSongCategory = createAsyncThunk(
    ...
    async (payload: AllSongPayload | undefined) => {
        return apiInstanceFetch.delete(`admin/songCategory/deleteSongCategory?songCategoryId=${payload?.songCategoryId}`);
    }
);
...
export const updateSongCategory = createAsyncThunk(
    ...
    async (payload: AllSongPayload | undefined) => {
        return axios.patch(`admin/songCategory/update?songCategoryId=${payload?.songCategoryId}`, payload.data);
    }
);
...
const songReducer = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(allSong.fulfilled, (state, action: PayloadAction<any, string, { arg: AllSongPayload }>) => {
        ...
        builder.addCase(deleteSong.fulfilled, (state, action: PayloadAction<any, string, { arg: AllSongPayload }>) => {
        ...
        builder.addCase(deleteSong.rejected, (state, action: PayloadAction<any>) => {
            state.isLoading = false;
        });
        ...
        builder.addCase(updateSong.fulfilled, (state, action: PayloadAction<any, string, { arg: AllSongPayload }>) => {
            state.isLoading = false;
            if (action.payload.data.status === true) {
                const songIndex = state.allSongData.findIndex((song) => song?._id === action.meta.arg?.songId);
                if (songIndex !== -1) {
                    state.allSongData[songIndex] = { ...state.allSongData[songIndex], ...action.payload.data?.data };
                }
        ...
        builder.addCase(allSongCategory.pending, (state, action: PayloadAction<any>) => {
            state.isLoading = true;
        });

        builder.addCase(allSongCategory.fulfilled, (state, action: PayloadAction<any, string, { arg: AllSongPayload }>) => {
            state.songCategoryData = action.payload.songCategory;
            state.totalSongCategory = action.payload.totalSongCategory;
            state.isLoading = false;
        });

        builder.addCase(allSongCategory.rejected, (state, action: PayloadAction<any>) => {
            state.isLoading = false;
        });

        builder.addCase(deleteSongCategory.pending, (state, action: PayloadAction<any>) => {
            state.isLoading = true;
        });

        builder.addCase(deleteSongCategory.fulfilled, (state, action: PayloadAction<any, string, { arg: AllSongPayload }>) => {
        ...
        builder.addCase(deleteSongCategory.rejected, (state, action: PayloadAction<any>) => {
            state.isLoading = false;
        });
        ...
        builder.addCase(addSongCategory.fulfilled, (state, action: PayloadAction<any>) => {
            state.isLoading = false;
            if (action.payload.data.status === true) {
        ...
        builder.addCase(updateSongCategory.fulfilled, (state, action: PayloadAction<any, string, { arg: AllSongPayload }>) => {
            state.isLoading = false;
            console.log(" action.meta.arg?.songCategoryId", action.meta.arg?.songCategoryId)
            if (action.payload.data.status === true) {
                const songIndex = state.songCategoryData.findIndex((song) => song?._id === action.meta.arg?.songCategoryId);
                if (songIndex !== -1) {
                    state.songCategoryData[songIndex] = { ...state.songCategoryData[songIndex], ...action.payload.data?.songCategory };
                }
