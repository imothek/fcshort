...
export const verificationRequestAccept = createAsyncThunk(
    ...
    async (payload: AllHashTagPayload | undefined) => {
        return axios.patch(`admin/verificationRequest/verificationRequestAccept?verificationRequestId=${payload?.verificationRequestId}`);
    }
);
...
export const verificationRequestDecline = createAsyncThunk(
    ...
    async (payload: AllHashTagPayload | undefined) => {
        return axios.patch(`admin/verificationRequest/verificationRequestDecline?verificationRequestId=${payload?.verificationRequestId}`, payload.data);
    }
);
...
const verificationRequestReducer = createSlice({
    ...
    extraReducers: (builder) => {
        builder.addCase(getVerificationRequest.pending, (state, action: PayloadAction<any>) => {
            state.isLoading = true;
        });

        builder.addCase(getVerificationRequest.fulfilled, (state, action: PayloadAction<any, string, { arg: AllHashTagPayload }>) => {
        ...
        builder.addCase(getVerificationRequest.rejected, (state, action: PayloadAction<any>) => {
            state.isLoading = false;
        });
        ...
        builder.addCase(verificationRequestAccept.fulfilled, (state, action: PayloadAction<any, string, { arg: AllHashTagPayload }>) => {
            state.isLoading = false;
            if (action.payload.data.status === true) {
                ...
                setToast("success", ` Verification Successfully`)
            }else{
        ...
        builder.addCase(verificationRequestDecline.fulfilled, (state, action: PayloadAction<any, string, { arg: AllHashTagPayload }>) => {
            state.isLoading = false;
            if (action.payload.data.status === true) {
                const deletedUserIds = action.meta.arg.verificationRequestId
                state.pendingData = state.pendingData.filter(
                ...
                setToast("success", ` Verification Declined Successfully`)
            }else{
                setToast("error",action.payload.data.message)
            }
        });
