...
export const deleteHastTag = createAsyncThunk(
    ...
    async (payload: AllHashTagPayload | undefined) => {
        return apiInstanceFetch.delete(`admin/hashTag/delete?hashTagId=${payload?.hashTagId}`);
    }
);
...
export const updateHashTag = createAsyncThunk(
    ...
    async (payload: AllHashTagPayload | undefined) => {
        return axios.patch(`admin/hashTag/update?hashTagId=${payload?.hashTagId}`, payload.data);
    }
);
...
const hashTagReducer = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(getHashtag.fulfilled, (state, action: PayloadAction<any, string, { arg: AllHashTagPayload }>) => {
            state.isLoading = false;
            state.allHashTagData = action.payload.data;
        });

        builder.addCase(getHashtag.rejected, (state, action: PayloadAction<any>) => {
            state.isLoading = false;
        });
        builder.addCase(deleteHastTag.pending, (state, action: PayloadAction<any>) => {
            state.isLoading = true;
        });

        builder.addCase(deleteHastTag.fulfilled, (state, action: PayloadAction<any, string, { arg: AllHashTagPayload }>) => {
        ...
        builder.addCase(deleteHastTag.rejected, (state, action: PayloadAction<any>) => {
            state.isLoading = false;
        });
        ...
        builder.addCase(addHashTag.fulfilled, (state, action: PayloadAction<any>) => {
            state.isLoading = false;
        ...
        builder.addCase(updateHashTag.fulfilled, (state, action: PayloadAction<any, string, { arg: AllHashTagPayload }>) => {
            state.isLoading = false;
            if (action.payload.data.status === true) {
                const hashTagIndex = state.allHashTagData.findIndex((hashTag) => hashTag?._id === action?.meta.arg?.hashTagId);
                if (hashTagIndex !== -1) {
                    state.allHashTagData[hashTagIndex] = { ...state.allHashTagData[hashTagIndex], ...action.payload?.data?.data };
                }
